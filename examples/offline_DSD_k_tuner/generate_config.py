...
from manager import DynamicSpeculativeConfig, DynamicSpeculativeDecodingManager
import sys
import os
# os.environ["ASCEND_RT_VISIBLE_DEVICES"] = "7"      # visible npu
...
def main():
    ...
    args.enforce_eager = False  # change to true
...
"""
time SETUPTOOLS_USE_DISTUTILS=stdlib python3 generate_config.py \
    --method eagle3 \
    --model-dir '/home/data/Qwen3-8B' \
    --draft-dir '/home/data/qwen3_8b_eagle3' \
    --tp 1 \
    --temp 0 \
    --top-p 1.0 \
    --top-k -1 \
    ...
    --num-speculative-tokens-list 1 3 5 7\
    --num-batches 20 \
    --dataset-name hf \
    --dataset-path '/home/c00943444/datasets/mt-bench' \
...
time SETUPTOOLS_USE_DISTUTILS=stdlib  python3 generate_config.py \
    ...
    --top-p 1.0 \
    --top-k -1 \
    --max-vllm-batch-size 16 \
    --batch-size-list 1 4 16 \
    --num-speculative-tokens-list 1 3 \
    --num-batches 5 \
    --dataset-name hf \
    --dataset-path '/home/c00943444/datasets/mt-bench' \
    --hf-name 'philschmid/mt-bench' \
